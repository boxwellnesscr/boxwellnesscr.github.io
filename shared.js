/* ============================================================
   BOXWELLNESS — shared.js v5
   NUEVO: selector de tamaño + sabor con cambio de imagen
   ============================================================ */
...
const CATEGORY_TREE = [
  ...
  { id:'creatinas', label:'CREATINAS', children:[
    {id:'creatinas-mono',label:'Monohidrato'},
    {id:'creatinas-hcl',label:'HCL'},
    {id:'creatinas-kre',label:'Kre-Alkalyn'},
    {id:'creatinas-otras',label:'Otras variedades'},
  ]},
...
function getCatFamily(catId){
  const p=CATEGORY_TREE.find(g=>g.id===catId);
  return p?[p.id,...p.children.map(c=>c.id)]:[catId];
}
...
function emptyWAHtml(filterActive){
  ...
  return`<div class="empty-state-wa"><div class="empty-icon">🏗️</div><p><strong>Estamos trabajando en estos productos.</strong> Podés consultar disponibilidad directamente por WhatsApp.</p><a href="https://wa.me/${BW.WA}?text=${msg}" class="empty-wa-btn" target="_blank">${waSVG()} Consultar disponibilidad</a></div>`;
}
...
/* ════════════════════════════════════════════════════════════
   IMAGE MATRIX SYSTEM
   ════════════════════════════════════════════════════════════
   Cada producto puede tener hasta 5 tamaños y 10+ sabores.
   El sistema busca la imagen más específica disponible:
     1. imagen[tamano][sabor]  → foto específica de ese tamaño+sabor
     2. imagen[tamano]         → foto genérica de ese tamaño
     3. imagen[sabor]          → foto genérica de ese sabor
     4. p.imagen               → foto principal del producto
     5. emoji                  → fallback

   Estructura en productos.json:
   {
     "imagen": "imagenes/001.jpg",           ← foto principal (fallback)
     "sabores": [
       { "nombre": "Chocolate 🍫", "imagen": "imagenes/001-choco.jpg" },
       { "nombre": "Vainilla 🍦",  "imagen": "" }
     ],
     "tamanios": [
       {
         "nombre": "60 servidas (300g)",
         "imagen": "imagenes/001-300g.jpg",       ← foto genérica de este tamaño
         "sabores": [
           { "sabor": "Chocolate 🍫", "imagen": "imagenes/001-300g-choco.jpg" },
           { "sabor": "Vainilla 🍦",  "imagen": "imagenes/001-300g-vainilla.jpg" }
         ]
       },
       {
         "nombre": "200 servidas (1KG)",
         "imagen": "imagenes/001-1kg.jpg",
         "sabores": [
           { "sabor": "Chocolate 🍫", "imagen": "imagenes/001-1kg-choco.jpg" },
           { "sabor": "Vainilla 🍦",  "imagen": "" }
         ]
       }
     ]
   }

   ¿Tenés solo tamaños pero no sabores? Dejás "sabores" vacío y ponés
   la imagen en tamanios[i].imagen directamente.

   ¿Tenés solo sabores pero no tamaños? Usás el array "sabores" simple
   (sin tocar tamanios) — igual que antes.
   ════════════════════════════════════════════════════════════ */

/* Resolves the best image for a given size+flavor combo */
function resolveImg(p, tamIdx, sabIdx) {
  // Try size+flavor combo first
  if (tamIdx !== null && sabIdx !== null) {
    const t = p.tamanios?.[tamIdx];
    if (t && Array.isArray(t.sabores)) {
      const ts = t.sabores.find((_,i)=>i===sabIdx) || t.sabores[sabIdx];
      if (ts && ts.imagen) return ts.imagen;
    }
  }
  // Try size-only image
  if (tamIdx !== null) {
    const t = p.tamanios?.[tamIdx];
    if (t && t.imagen) return t.imagen;
  }
  // Try flavor-only image (simple sabores array)
  if (sabIdx !== null) {
    const s = p.sabores?.[sabIdx];
    if (s && s.imagen) return s.imagen;
  }
  // Fallback to product main image
  return p.imagen || '';
}

/* Detect if tamanios is new object format or old string array */
function isTamObj(p) {
  return p.tamanios && p.tamanios.length > 0 && typeof p.tamanios[0] === 'object';
}
function getTamNombre(t) { return typeof t === 'object' ? t.nombre : t; }

/* State for current modal selections */
let _pmTamIdx  = null;
let _pmSabIdx  = null;
let _pmProduct = null;

function pmUpdateImage() {
  const wrap = document.getElementById('pmMainImgWrap'); if(!wrap) return;
  const img = resolveImg(_pmProduct, _pmTamIdx, _pmSabIdx);
  if(img) wrap.innerHTML = `<img id="pmMainImg" src="${img}" alt="">`;
  else {
    const emoji = _pmProduct.emoji || '📦';
...
window.pmChangeTamanio = function(sel) {
  ...
  /* If new format has size-specific flavors, rebuild flavor select */
  if (isTamObj(_pmProduct) && _pmTamIdx !== null) {
    const tam = _pmProduct.tamanios[_pmTamIdx];
    if (tam.sabores && tam.sabores.length > 0) {
      const saborSel = document.getElementById('pmSaborSelect');
      if (saborSel) {
        saborSel.innerHTML = `<option value="">Elige un sabor</option>` +
          tam.sabores.map((s,i)=>`<option value="${i}">${typeof s==='object'?s.sabor:s}</option>`).join('');
        _pmSabIdx = null;
      }
    }
  }
...
window.pmChangeSabor = function(sel) {
  const idx = parseInt(sel.value);
...
window.pmSwapImg = function(thumb) {
  ...
  wrap.innerHTML=img?`<img id="pmMainImg" src="${img}" alt="">`:
    `<div id="pmMainImg" class="pm-emoji-placeholder">${thumb.dataset.emoji||'📦'}</div>`;
};

/* ── PRODUCT MODAL ─────────────────────────────────────────── */
function openProductModal(id){
  const p=(window._allProducts||[]).find(x=>x.id===id);if(!p)return;
  const modal=document.getElementById('productModal'),inner=document.getElementById('productModalInner');
  if(!modal||!inner)return;

  /* Reset state */
  _pmProduct=p; _pmTamIdx=null; _pmSabIdx=null;

  /* Initial image: first tamanio + first sabor if available */
  if(isTamObj(p) && p.tamanios.length>0){
    _pmTamIdx=0;
    if(p.tamanios[0].sabores && p.tamanios[0].sabores.length>0) _pmSabIdx=0;
  } else if(p.sabores && p.sabores.length>0){
    _pmSabIdx=0;
  }
  const initImg = resolveImg(p, _pmTamIdx, _pmSabIdx) || p.imagen || '';
  const mainContent = initImg
    ? `<img id="pmMainImg" src="${initImg}" alt="${p.nombre}">`
    : `<div id="pmMainImg" class="pm-emoji-placeholder">${p.emoji||'📦'}</div>`;

  /* Thumbs: gather unique images from sabores (simple) + imagenNutricional */
  const thumbs=[];
  if(!isTamObj(p) && p.sabores) {
    p.sabores.forEach((s,i)=>{
      thumbs.push(s.imagen
        ?`<div class="pm-thumb ${i===0?'active':''}" data-img="${s.imagen}" onclick="pmSwapImg(this)"><img src="${s.imagen}" loading="lazy"></div>`
        :`<div class="pm-thumb ${i===0?'active':''}" data-img="" data-emoji="${p.emoji||'📦'}" onclick="pmSwapImg(this)"><span class="pm-thumb-emoji">${p.emoji||'📦'}</span></div>`);
    });
  }
  ...
  /* ── TAMAÑOS SELECTOR ── */
  let tamaniosEl='';
  if(p.tamanios && p.tamanios.length>0){
    if(p.tamanios.length===1){
      tamaniosEl=`<div class="pm-variant-group"><label class="pm-variant-label">Presentación</label><span class="pm-variant-tag">${getTamNombre(p.tamanios[0])}</span></div>`;
    } else {
      tamaniosEl=`<div class="pm-variant-group">
        <label class="pm-variant-label">Tamaño</label>
        <select class="pm-variant-select" id="pmTamanioSelect" onchange="pmChangeTamanio(this)">
          <option value="">Elige un tamaño</option>
          ${p.tamanios.map((t,i)=>`<option value="${i}" ${i===0?'selected':''}>${getTamNombre(t)}</option>`).join('')}
        </select>
      </div>`;
    }
  }
  ...
  /* ── SABORES SELECTOR ──
     If new object tamanios: show flavors from first size.
  ...
  if(initFlavors.length>1){
    saboresEl=`<div class="pm-variant-group">
      <label class="pm-variant-label">Sabor</label>
      <select class="pm-variant-select" id="pmSaborSelect" onchange="pmChangeSabor(this)">
        <option value="">Elige un sabor</option>
        ${initFlavors.map((s,i)=>`<option value="${i}" ${i===0?'selected':''}>${typeof s==='object'?(s.sabor||s.nombre):s.nombre}</option>`).join('')}
      </select>
    </div>`;
  } else if(initFlavors.length===1){
    const name=typeof initFlavors[0]==='object'?(initFlavors[0].sabor||initFlavors[0].nombre):initFlavors[0].nombre;
    saboresEl=`<div class="pm-variant-group"><label class="pm-variant-label">Sabor</label><span class="pm-variant-tag">${name}</span></div>`;
  }
...
function initSearch(onSearch){
  ...
  inp?.addEventListener('input',()=>{clearTimeout(_sd);_sd=setTimeout(()=>{const q=inp.value.trim();if(!isProductsPage&&q){window.location.href=`productos.html?search=${encodeURIComponent(q)}`}else{onSearch&&onSearch(inp.value)}},300)});
  inp?.addEventListener('keydown',e=>{if(e.key==='Enter'){const q=inp.value.trim();if(q)window.location.href=`productos.html?search=${encodeURIComponent(q)}`}});
}
...
function _fallback(){return[
  {id:'001',nombre:'Nutrex Creatina Monohidrato',marca:'NUTREX',precio:18500,precioAnterior:22000,categorias:['creatinas'],descripcion:'Creatina monohidrato pura.',emoji:'🔬',imagen:'',imagenNutricional:'',
   sabores:[{nombre:'Sin sabor 🥄',imagen:''},{nombre:'Fruit Punch 🍉',imagen:''},{nombre:'Strawberry Watermelon 🍓',imagen:''}],
   tamanios:[{nombre:'60 servidas (300g)',imagen:'',sabores:[{sabor:'Sin sabor 🥄',imagen:''},{sabor:'Fruit Punch 🍉',imagen:''}]},{nombre:'100 servidas (500g)',imagen:'',sabores:[{sabor:'Sin sabor 🥄',imagen:''}]},{nombre:'200 servidas (1KG)',imagen:'',sabores:[{sabor:'Sin sabor 🥄',imagen:''}]}],
   caracteristicas:['5g de creatina monohidrato por servicio','Fórmula micronizada'],modoDeUso:'Mezclar 1 scoop en 250ml de agua.',badge:'oferta',destacado:true,disponible:true},
  {id:'002',nombre:'Dymatize ISO 100 5LB',marca:'DYMATIZE',precio:68000,precioAnterior:75000,categorias:['proteinas','isoladas'],descripcion:'Isolada premium hidrolizada.',emoji:'🥛',imagen:'',imagenNutricional:'',
   sabores:[{nombre:'Fudge Brownie 🍫',imagen:''},{nombre:'Gourmet Vanilla 🍦',imagen:''},{nombre:'Birthday Cake 🎂',imagen:''}],
   tamanios:[{nombre:'20 servidas (1.3lb)',imagen:'',sabores:[{sabor:'Fudge Brownie 🍫',imagen:''},{sabor:'Gourmet Vanilla 🍦',imagen:''}]},{nombre:'42 servidas (3lb)',imagen:'',sabores:[{sabor:'Fudge Brownie 🍫',imagen:''},{sabor:'Gourmet Vanilla 🍦',imagen:''},{sabor:'Birthday Cake 🎂',imagen:''}]},{nombre:'76 servidas (5lb)',imagen:'',sabores:[{sabor:'Fudge Brownie 🍫',imagen:''},{sabor:'Gourmet Vanilla 🍦',imagen:''},{sabor:'Birthday Cake 🎂',imagen:''}]}],
   caracteristicas:['25g proteína isolada hidrolizada','0g azúcar','Libre de gluten'],modoDeUso:'Mezclar 1 scoop en 250ml de agua fría post-entreno.',badge:'oferta',destacado:true,disponible:true},
]}
